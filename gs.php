...
//Should we output the result to a CSV file?
//Note: csv file does not have column headers. they must be added manually.
//The columns are: number of pairs, man match goodness, woman match goodness
$output_to_csv = true;
...
function check_for_remaining_free_men()
{
    ...
    foreach ($free_men as $freeman) {
        if (count($freeman->get_unique_proposal_list()) < $number_of_pairs) {
            return true;
        }
    }
...
while (check_for_remaining_free_men()) {
    $loops++;
    echo "\n\n\nLoop $loops\nCurrent memory usage: " . (round(memory_get_usage() / 1024)) . "mb";
...
if ($output_to_csv) {
    $output = '"' . $number_of_pairs . '",' . '"' . (round(($m_goodness_sum / $number_of_pairs), 3)) . '",' . '"' . (round(($w_goodness_sum / $number_of_pairs), 3)) . '"';
    if (file_put_contents('gs-output.csv', $output . PHP_EOL, FILE_APPEND | LOCK_EX))
        echo "\n\nResults appended to gs-output.csv successfully.";
}
